...
using Infected = std::unordered_map<std::pair<int, int>, State, PairHash>;
...
auto get_infected_from_map(const auto &map)
{
	...
	for (int y = 0; y < map.size(); ++y) {
		for (int x = 0; x < map[y].size(); ++x) {
			if (map[y][x] == '#') {
				infected[{x - mid_x, mid_y - y}] = State::infected;
			}
		}
	}
...
int main()
{
	...
	for (std::size_t bursts = 0; bursts != 10'000'000; ++bursts) {
		if (auto [it, success] = infected.insert({{x, y}, State::weakened}); !success) {
			switch (it->second) {
			case State::weakened:
